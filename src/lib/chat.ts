...
export async function getChatResponse(deploymentName: string, messages: ChatMessage[], config?: Partial<OpenAIChatPayload>): Promise<OpenAIChatResponse> {
  const payload = {
    messages,
    temperature: 0.7,
    top_p: 1,
  ...
  try {
    const result: OpenAIChatResponse = await fetch(`/proxy/openai/deployments/${deploymentName}/chat/completions?api-version=2024-02-15-preview`, {
      method: "post",
      headers: {
...
export async function* getChatStream(
  ...
  const stream = await fetch(`/proxy/openai/deployments/${deploymentName}/chat/completions?api-version=2024-02-15-preview`, {
    method: "post",
    headers: {
      "Content-Type": "application/json",
    },
